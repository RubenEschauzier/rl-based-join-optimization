...
class EvalWithOptimalLoggingCallback(MaskableEvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, episode_infos = self._evaluate_policy(
    ...
    def _on_training_end(self) -> None:
        ...
        if self.optimal_rewards:
            avg_opt = sum(self.optimal_rewards) / len(self.optimal_rewards)
            self.logger.record("final/avg_optimal_reward", avg_opt)
        if self.agent_rewards:
            avg_ep = sum(self.agent_rewards) / len(self.agent_rewards)
            self.logger.record("final/avg_episode_reward", avg_ep)
        self.logger.dump(self.num_timesteps)

    @staticmethod
    def _evaluate_policy(
            model: MaskablePPO | MaskableQRDQN,
